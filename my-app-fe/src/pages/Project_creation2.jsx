...
function ProjectCreation2() {
    ...
    function handleSubmit() {
        ...
        if (incompleteAssignments.length > 0) {
            alert(`Please fill in all fields for the following assignments:\n${incompleteAssignments.join(", ")}`);
            return;
        }

        const assignmentsData = assignments.map((assignment, index) => ({
            ...assignment,
            id: index + 1, // Add an ID for testing purposes
        }));

        console.log("Assignments Data:", assignmentsData);
        localStorage.setItem("assignmentsData", JSON.stringify(assignmentsData)); // Save data to local storage
        alert("Assignments saved for testing purposes.");
        navigate("/main");
    }
